...
function Cell({
  ...
  const getShipPreviewCells = React.useCallback(() => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      if (isMyBoard && myBoard[r][c] !== "E") {
        // myBoard comes from parent Tablero props now
        canPlace = false;
        break;
      }
      ...
      if (isMyBoard && hasAdjacent(myBoard, c, r)) {
        // Check if cell is adjacent to an existing ship
        canPlace = false;
        break;
      }
  ...
  const handleDrop = (e) => {
    ...
    if (isMyBoard && isPlacingShipsManually && draggingShip) {
      ...
      if (
        isValidPlacement(newBoard, shipSize, orientation, startRow, startCol)
      ) {
        ...
        alert(
          "No se puede colocar el barco aquí. Asegúrate de que haya espacio y no toque otros barcos."
        );
      }
  ...
  return (
    <div
      ...
      className={`${cellClass} ${
        isCellPartOfPreview(rowIndex, colIndex) ? "ship-preview" : ""
      }`}
      onClick={
      ...
      style={{
        backgroundColor: isInRange(rowIndex, colIndex, cellData)
          ? "#ffc107"
          : isCellPartOfPreview(rowIndex, colIndex)
          ? "rgba(0, 255, 0, 0.5)"
          : null, // Green for preview
      }}
      onMouseEnter={() => {
