...
const ArsenalPanel = () => {
  // Estado para controlar qué arsenal está seleccionado
  const [arsenalSeleccionado, setArsenalSeleccionado] = useState(null); // Inicialmente nada seleccionado
    ...
    <div className="arsenal-panel-container">
      ...
      <div className="arsenal-buttons">
        {arsenales.map((arsenal) => (
          ...
          >
            ...
            <img src={arsenal.icon} alt={arsenal.name} className="arsenal-icon" />
      ...
      {arsenalSeleccionado && (
        <p className="selected-info">
          Has seleccionado: <span className="selected-item">{arsenalSeleccionado.toUpperCase()}</span>
        </p>
      )}
    </div>
  );
